...
class MoveBaseSeq():
    ...
    def __init__(self):
        ...
        points_seq = [-4,3,0,  0,7,0,  -4,3,0,  0,7,0,  -4,3,0,  0,7,0]
       
        
        n = 3
        # Returns a list of lists [[point1], [point2],...[pointn]]
        points = [points_seq[i:i+n] for i in range(0, len(points_seq), n)]                  #[[-4, 3, 0], [0, 7, 0], [-4, 3, 0], [0, 7, 0], [-4, 3, 0], [0, 7, 0]]
        a = [[0,0,0]]
        a.extend(points)
        
        # Only yaw angle required (no ratotions around x and y axes) in deg:
        # yaweulerangles_seq = rospy.get_param('move_base_seq/yea_seq')
        yaweulerangles_seq = [np.arctan2(a[i+1][1]-a[i][1], a[i+1][0] - a[i][0]) for i in range(len(points))]
        ...
        wait = self.client.wait_for_server(rospy.Duration(0.2))
        if not wait:
