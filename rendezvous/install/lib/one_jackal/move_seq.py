...
class MoveBaseSeq():
    ...
    def __init__(self):
        ...
        # if not wait:
        #     rospy.logerr(self.robot_name + ": Action server not available!")
        #     rospy.signal_shutdown(self.robot_name + ": Action server not available!")
        #     return
    ...
    def sub_cb(self, data):
        ...
        # point = [self.nodeList[0][0], self.nodeList[0][1]]

        self.sendGoal(self.nodeList[0], self.done_cb, self.yaw[0])
        # rospy.loginfo(self.robot_name + ": WayPoints and Goal: " + str(self.nodeList))
        self.nodeList = np.delete(self.nodeList, 0, 0)
    ...
    def done_cb(self, status, result):
        ...
        if status == 3:
            if self.nodeList.shape[0] > 0:
                # rospy.loginfo(self.robot_name + ": Next Point: " + str(self.nodeList[0]))
                # point = [self.nodeList[0][0], self.nodeList[0][1]]
                self.sendGoal(self.nodeList[0], self.done_cb, self.yaw[0])
                self.nodeList = np.delete(self.nodeList, 0, 0)
                self.yaw = np.delete(self.yaw, 0)
    ...
    def init1_cb(self, status, result):
        self.sendGoal([0,1,0], self.init2_cb, np.pi)

    def init2_cb(self, status, result):
        self.sendGoal([0,0,0], self.done_cb, 0)
