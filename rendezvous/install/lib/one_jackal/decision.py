...
class NavSelection():
    ...
    def __init__(self):
        ...
        param_name = rospy.search_param('lambda')
        self.saveinfo['lambda'] = str(rospy.get_param(param_name))
    ...
    def node_cb(self, data):
        ...
        if rospy.get_time() - self.inittime > self.timelimit:
        ...
        cwd = os.getcwd()
        filename = str(cwd) + "/info/Gstar/graph_"+str(self.step) +".png"
    ...
    def collisioncheck(self):
        ...
        for i in range(self.nodes.shape[0]):
            ...
            for j in range(50):
                ...
                cx = np.cos(angle) * 5 + nix
                cy = np.sin(angle) * 5 + niy
    ...
    def pathplaning(self):
        if self.saveinfo['lambda'] == 'avarage edge length':
            self.lamb = self.get_avglength()
        elif self.saveinfo['lambda'] == 'far':
            self.lamb = 0
        elif self.saveinfo['lambda'] == '3':
            self.lamb = 3
        else:
            self.lamb = 100

        self.curPose = self.navi_goal
        # rospy.loginfo(str(self.robot_name)+ ": Current Node:"+str(self.curPose))
        # rospy.loginfo(str(self.robot_name)+ ": Current Visited:\n"+str(self.visited))

        # rospy.loginfo(str(self.robot_name)+ ": Graph shape:\n"+str(self.edges.shape))
        ...
        for i in range(dist.shape[0]):
            if i!=self.curPose:
                ...
                for j in range(len(path)):
                    weight[i] -= self.lamb * self.visited[path[j]]
        ...
        if not self.stop:
            ...
            # rospy.loginfo(str(self.robot_name)+ ": wayindex:"+str(self.wyidx))
            # rospy.loginfo(str(self.robot_name)+ ": waypoints:\n"+str(waypoints))
    ...
    def plot_graph(self):
        ...
        for i in range(self.nodes.shape[0]):
            ...
            # rospy.loginfo(str(self.robot_name)+ ": color:"+str(col))
        ...
        plt.text(xmax+1,ymax-2,s='lambda: ' + str(round(self.lamb,2)),                                      c='red', fontsize = 12)
    ...
    def get_avglength(self):
        ...
        for i in range(self.edges.shape[0]):
            for j in range(self.edges.shape[1]):
                if self.edges[i][j] > 0:
                    numOfedge += 1
                sumOflength += self.edges[i][j]
        return sumOflength / numOfedge

    def updatesaveinfo(self, res):
        self.saveinfo['timesc'] = str(rospy.get_time() - self.inittime)
        self.saveinfo['timest'] = self.step
        self.saveinfo['distance'] = str(self.distTravel)
